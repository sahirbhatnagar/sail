...
Many of the linear effect methods consider all pairwise interactions in $\bX$~\citep{zhao2009composite,choi2010variable,bien2013lasso, she2014group,hao2018model,shah2016modelling} while some allow the user to restrict the search to interaction candidates~\citep{lim2015learning,haris2016convex}. 
There is not much literature for non-linear interactions. Our proposal is most similar to the pliable lasso which can also be viewed as a varying coefficient model. 
...
<<simulation-results, dev="cairo_pdf", fig.asp=1, fig.width=12, fig.heigh=11, fig.pos = 'H', eval=F, fig.cap="Test set MSE results.">>=
@
