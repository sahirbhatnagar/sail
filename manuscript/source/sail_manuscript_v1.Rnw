...
In this paper, we propose a multivariable penalization procedure for detecting linear and non-linear interactions between high dimensional data $\mb{X}$ and a single environmental factor $E$ on a response vector $Y$. 
...
Specifically, we are interested in imposing the strong heredity principle~\citep{chipman1996bayesian}: 
\begin{equation}
