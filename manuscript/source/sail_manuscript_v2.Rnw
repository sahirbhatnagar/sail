...
Our algorithms are in an R package available on CRAN.
\end{abstract}
...
Fifth, we implement our algorithms in the \texttt{sail} R package on CRAN, along with extensive documentation available at \url{http://sahirbhatnagar.com/sail/}. 
