...
<<toy-solution-path, fig.cap="Toy example solution path for main effects (top) and interactions (bottom). $\\lambda_{1SE}$ is the largest value of penalization for which the CV error is within one standard error of the minimizing value $\\lambda_{min}$. ", fig.asp=0.8, fig.pos = "H", eval=finalversion>>=
@
...
<<toy-effects, fig.cap="Estimated smooth functions by \\texttt{sail} method based on $\\lambda_{min}$.", fig.pos = "H", eval=finalversion>>=
@
