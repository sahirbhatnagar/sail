...
In this section we study the asymptotic behaviour of the \sail ~estimator $\widehat{\bTheta}$, defined as the minimizer of~\eqref{eq:saillikelihood}, as well as the model selection properties. We show that $\texttt{{sail}}$ posesses the oracle property when the sample size approaches infinity and the number of predictors is fixed. That is, under certain regularity conditions, it performs as well as if the true model were known in advance and has the optimal estimation rate~\citep{zou2006adaptive}. Let $\bThetastar = (\beta_{E}^*,\btheta_{1}^*,\ldots,\btheta_{p}^*,\btau_{1}^*,\ldots,\btau_{p}^*)$ denote the unknown vector of true coefficients and $\mathcal{H}=\left\lbrace h : \bThetastar_h \neq 0 \right\rbrace$ the unknown sparsity pattern of $\bThetastar$. Note that our asymptotic results are stated for the main effects and interaction terms only, even though our formulation includes an unpenalized intercept. In the least-squares setting, 

\[
\mathcal{L}(\bTheta|\bD)=\frac{1}{2n}\left\Vert Y-\beta_{0}\cdot\boldsymbol{1}-\sum_{j=1}^{p}\bPsi_{j}\btheta_{j}-\beta_{E}X_{E}-\sum_{j=1}^{p}(X_{E}\circ\bPsi_{j})\btau_{j}\right\Vert _{2}^{2}
\]
...
\[
\begin{aligned}\widehat{\bTheta}= & \argmin_{\bTheta}Q(\bTheta)\end{aligned}
=\mathcal{L}(\bTheta|\bD)+n\lambda_{E}\left|\beta_{E}\right|+n\sum_{j=1}^{p}\lambda_{j}^{\theta}\left\Vert \btheta_{j}\right\Vert _{2}+n\sum_{k=1}^{p}\lambda_{k}^{\gamma}\left|\gamma_{k}\right|
\]
where $\mathcal{L}(\bTheta|\bD)$ is the negative log-likelihood,
$\lambda_{E}=\frac{{1}}{n}\lambda(1-\alpha)w_{E}$, $\lambda_{j}^{\theta}=\frac{{1}}{n}\lambda(1-\alpha)w_{j}$
, and $\lambda_{k}^{\gamma}=\frac{{1}}{n}\lambda\alpha w_{kE}$. 

Let $\eta_{n}=n^{-1/2}+a_{n}$ and $\{\bTheta^{*}+\eta_{n}\boldsymbol{\delta}:\|\boldsymbol{\delta}\|\leq d\}$
be the ball around $\bTheta^{*}$, where $\boldsymbol{\delta}=(u_{E},\boldsymbol{u}_{1,}\ddd\boldsymbol{u}_{p},v_{1E},\ldots,v_{p,E})^{\top}=(\boldsymbol{u}^{\top},\boldsymbol{v}^{\top})^{\top}$.
Define 
\[
D_{n}(\boldsymbol{\delta})\equiv Q_{n}(\bThetastar+\eta_{n}\bdelta)-Q_{n}(\bThetastar).
\]
Let $-L_{n}$ denote the first term of $Q_{n}$ in (8). For $\bdelta$
that satisfies $\|\bdelta\|=d$, we have
\begin{align*}
D_{n}(\bdelta) & =-L_{n}(\bThetastar+\eta_{n}\bdelta)+L_{n}(\bThetastar)+n\lambda_{E}(|\beta_{E}^{*}+\eta_{n}u_{E}|-|\beta_{E}^{*}|)+n\sum_{j}\lambda_{j}^{\theta}\left(\left\Vert \btheta{}_{j}^{*}+\eta_{n}\boldsymbol{u}_{j}\right\Vert _{2}-\left\Vert \btheta_{j}^{*}\right\Vert _{2}\right)\\
& +n\sum_{k}\lambda_{k}^{\gamma}(|\gamma_{k}^{*}+\eta_{n}v_{kE}|-|\gamma_{k}^{*}|)\\
& \overset{(a)}{\geq}-L_{n}(\bThetastar+\eta_{n}\bdelta)+L_{n}(\bThetastar)+n\lambda_{E}(|\beta_{E}^{*}+\eta_{n}u_{E}|-|\beta_{E}^{*}|)\cdot I(\beta_{E}^{*}\neq0)+n\sum_{j\in\mathcal{A}_{2}}\lambda_{j}^{\theta}\left(\left\Vert \btheta{}_{j}^{*}+\eta_{n}\boldsymbol{u}_{j}\right\Vert _{2}-\left\Vert \btheta_{j}^{*}\right\Vert _{2}\right)\\
...
\end{align*}
Inequality (a) is by the fact that $\sum_{j\notin\mathcal{A}}|\theta_{j}^{*}|=0$.
(b) is due to reverse triangle inequality $|a|-|b|\geq-|a-b|$. (c)
is by $\lambda_{E}\leq\eta_{n}$, $\lambda_{j}^{\theta}\leq\eta_{n}$
and $\lambda_{k}^{\gamma}\leq\eta_{n}$. (d) is by Cauchy\textendash Schwarz
inequality
\[
\sum_{j\in\A_{1}}|u_{j}|=|\mathbf{1}_{\A_{1}}^{\top}\mathbf{u}_{\A_{1}}|\leq\|\mathbf{1}_{\A_{1}}\|\|\mathbf{u}_{\A_{1}}\|=\sqrt{|\A_{1}}|\|\mathbf{u}_{\A_{1}}\|\leq|\A_{1}|d.
\]
By the standard argument on the Taylor expansion of the loss function,
we have 
\begin{align*}
...
\[
\begin{aligned}A_{1} & =-\left[\nabla L_{n}\left(\boldsymbol{\theta}^{*}\right)\right]^{\mathrm{T}}\left(\eta_{n}\boldsymbol{\delta}\right)\\
A_{2} & =-\frac{1}{2}\left(\eta_{n}\boldsymbol{\delta}\right)^{\top}\left[\nabla^{2}L_{n}\left(\boldsymbol{\theta}^{*}\right)\right]\left(\eta_{n}\boldsymbol{\delta}\right)\left(1+o_{p}(1)\right)\\
A_{3} & =-n\eta_{n}^{2}\left(\left|\mathcal{A}_{1}\right|+\left|\mathcal{A}_{2}\right|\right)d
\end{aligned}
\]
Then
\[
\begin{aligned}A_{1} & =-\eta_{n}\left[\nabla L_{n}\left(\boldsymbol{\theta}^{*}\right)\right]^{\top}\boldsymbol{\delta}\\
& =-\sqrt{n}\eta_{n}\left(\frac{1}{\sqrt{n}}\nabla L_{n}\left(\boldsymbol{\theta}^{*}\right)\right)^{\top}\boldsymbol{\delta}\\
& =-\sqrt{n}\eta_{n}\left(\sqrt{n}\frac{1}{n}\sum_{i=1}^{n}\nabla\log f\left.\left(\boldsymbol{V}_{i},\boldsymbol{\theta}\right)\right|_{\boldsymbol{\theta}=\boldsymbol{\theta}^{*}}\right)^{\top}\boldsymbol{\delta}\\
& =-\sqrt{n}\eta_{n}\left(\sqrt{n}\left[\frac{1}{n}\sum_{i=1}^{n}\nabla\log f\left.\left(\boldsymbol{V}_{i},\boldsymbol{\theta}\right)\right|_{\boldsymbol{\theta}=\boldsymbol{\theta}^{*}}-\mathbf{0}\right]\right)^{\top}\boldsymbol{\delta}\\
& =-\sqrt{n}\eta_{n}\left(\sqrt{n}\left[\frac{1}{n}\sum_{i=1}^{n}\nabla\log f\left.\left(\boldsymbol{V}_{i},\boldsymbol{\theta}\right)\right|_{\boldsymbol{\theta}=\boldsymbol{\theta}^{*}}-E_{\boldsymbol{\theta}^{*}}\nabla L\left(\boldsymbol{\theta}^{*}\right)\right]\right)^{\top}\boldsymbol{\delta}\\
...
\end{align*}
\[
\begin{aligned}A_{2} & =\frac{1}{2}n\eta_{n}^{2}\left\{ \boldsymbol{\delta}^{\top}\left[-\frac{1}{n}\nabla^{2}L_{n}\left(\boldsymbol{\theta}^{*}\right)\right]\boldsymbol{\delta}\right\} \left(1+o_{p}(1)\right)\\
& =\frac{1}{2}n\eta_{n}^{2}\left\{ \boldsymbol{\delta}^{\top}\left[I\left(\boldsymbol{\theta}^{*}\right)\right]\boldsymbol{\delta}\right\} \left(1+o_{p}(1)\right)\text{ by the weak law of large numbers. }
\end{aligned}
\]
Thus,
\[
\begin{aligned}D_{n}(\boldsymbol{\delta}) & \geq A_{1}+A_{2}+A_{3}\\
& =-n\eta_{n}^{2}O_{p}(1)\boldsymbol{\delta}+\frac{1}{2}n\eta_{n}^{2}\left\{ \boldsymbol{\delta}^{\top}\left[I\left(\boldsymbol{\theta}^{*}\right)\right]\boldsymbol{\delta}\right\} \left(1+o_{p}(1)\right)-n\eta_{n}^{2}\left(\left|\mathcal{A}_{1}\right|+\left|\mathcal{A}_{2}\right|\right)d
\end{aligned}
\]
Notice that $A_{2}$ dominates the rest terms $A_{1}$ and $A_{3}$
and is positive since $I(\boldsymbol{\theta})$ is positive definite
at $\boldsymbol{\theta}=\bThetastar$ from (C2). Therefore, for any
given $\varepsilon>0$, there exists a large enough constant $d$
such that 
\[
P\left\{ \underset{\|\delta\|=d}{\inf}Q_{n}\left(\boldsymbol{\theta}^{*}+\eta_{n}\boldsymbol{\delta}\right)>Q_{n}\left(\boldsymbol{\theta}^{*}\right)\right\} \geq1-\varepsilon
\]
This implies with probability at least $1-\varepsilon$ that the empirical
likelihood $Q_{n}$ has a local minimizer in the ball $\{\boldsymbol{\theta}^{*}+\eta_{n}\mathbf{\bdelta}:\|\mathbf{\bdelta}\|\leq d\}$
(since $Q_{n}$ is bounded and $\{\boldsymbol{\theta}^{*}+\alpha_{n}\bdelta:\|\bdelta\|\leq d\}$
is closed). In other words, there exists a local maximizer $\widehat{\btheta}_{n}$
such that $\|\widehat{\boldsymbol{\theta}}_{n}-\boldsymbol{\theta}^{*}\|\leq\eta_{n}\|\bdelta\|\leq\eta_{n}d=\Op(\eta_{n})=\Op(n^{-1/2}+a_{n})$.
Hence, $\left\Vert \widehat{\boldsymbol{\theta}}_{n}-\boldsymbol{\theta}^{*}\right\Vert =O_{p}\left(\eta_{n}\right).\square$
...
<<plot-tpr-fpr-sim, dev="cairo_pdf", fig.asp=1, fig.width=12, fig.heigh=11, fig.pos = 'H', fig.cap="Means $\\pm 1$ standard deviation of true positive rate vs. false positive rate from 200 simulations for each of the five scenarios. $|S_0|$ is the number of truly associated variables.", eval=finalversion>>=
@
