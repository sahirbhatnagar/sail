...
knitr::read_chunk("../bin/support.R")
...
<<load-support, eval=T, results = 'hide'>>=
@
...
<<dzclass-interaction, fig.width=12, fig.height=8, fig.pos= 'H', fig.cap="Illustration of estimated interaction effects identified by \\texttt{sail} for the SUPPORT data. Coefficients estimated using the train/validate/test split which led to the best test set AUC was selected to derive the marginal interaction effects. Coefficients based on other train/validate/test splits were used to derive a 90% confidence interval colored in corresponding light colors.", eval=finalversion, results='hide'>>=
@
