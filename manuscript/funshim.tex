...
\begin{align}
g(\bmu)  = & \beta^*_0 + \sum_{j=1}^p f^*_j (X_{j}) + \beta^*_E E + \sum_{j=1}^p \gamma_{j}  \beta_E E f^*_j (X_{j})     \label{eq:linpred}
\end{align}
...
\end{equation}
where the $\left\lbrace \psi_{j\ell} \right\rbrace_1^{p_j}$ are a family of basis functions in $X_j$~\citep{hastie2015statistical} and \mbox{$\bbeta_j = (\beta_{j1}, \ldots, \beta_{jp_j})$} for $j = 1, \ldots, p$. 
...
This leads to the following penalized least squares criterion:
\begin{equation}
\argmin_{\beta_0, \bbeta, \bs{\gamma} }  \frac{1}{2} \norm{Y - g(\bmu)}^2 + \lambda_\beta w_E \norm{\beta_E}_2 + \lambda_{\beta}\sum_{j=1}^{p} w_j \norm{\bbeta_j}_2 +  \lambda_\gamma \sum_{j=1}^{p} w_{jE} \abs{\gamma_{jE}} \label{eq:lassolikelihood3}
\end{equation} 
