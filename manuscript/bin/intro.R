...
gendataIntro <- function (n, p, corr = 0, E = truncnorm::rtruncnorm(n, a = -1, b = 1),
  ...
  # W <- replicate(n = p, truncnorm::rtruncnorm(n, a = -2.5, b = 2.5))
  # U <- truncnorm::rtruncnorm(n, a = -2.5, b = 2.5)
  # V <- truncnorm::rtruncnorm(n, a = -2.5, b = 2.5)
  ...
  # Xall <- replicate(n = p, truncnorm::rtruncnorm(n, a = -2.5, b = 2.5))
  # Xall <- replicate(n = p, runif(n, min = -2.5, max = 2.5))
  # colnames(Xall) <- paste0("X", seq_len(p))
  #
  # X1 <- Xall[,1]
  # X2 <- Xall[,2]
  ...
  # f1 <- function(x) 3 * sin(2 * pi * x)/(2 - sin(2 * pi * x))
  # f3 <- function(x) 2.5 * sin(1.5 * x)
  # f2 <- function(x) 2 * (0.1 * sin(2 * pi * x) + 0.2 *
  #                          cos(2 * pi * x) + 0.3 * sin(2 * pi * x)^2 + 0.4 *
  #                          cos(2 * pi * x)^3 + 0.5 * sin(2 * pi * x)^3)

  # f3 <- function(x) 2.5 * sin(2 * pi * x)
  # f3 <- function(x) -2 * (2 * x - 1)^3
  # f3 <- function(x) exp(x)
  # f1 <- function(x) -2 * dnorm(x, mean = 0.5, sd = 0.8)
  # f1 <- function(x) sin(exp(-0.5*x))
  # f2 <- function(x) 0.25 * x^3 + 0.5*(x - 0.5)^2
  # f1 <- function(x) -2 * sin(1.5 * x)
...
lin_pred <- DT$betaE * DT$e + 1.5 * DT$f2.inter(DT$X2, DT$e)

lin_pred <- coef(cvfit, s="lambda.min")["E",] * DT$e + cvfit$sail.fit$design[,c("X2_1:E", "X2_2:E", "X2_3:E")] %*% coef(cvfit, s="lambda.min")[c("X2_1:E", "X2_2:E", "X2_3:E"),,drop=F]
...
plot(x2, lin_pred,
     pch = 19,
     ylab = latex2exp::TeX("$\\beta_E \\cdot E + f(X_2) \\cdot E$"),
     ...
     main = "",
     cex.main = 2.5,
     ylim = c(min.length.top-1, max.length.top+1))
axis(1, labels = T, cex.axis = 2)
lines(x2e0[order(x2e0)],e0[order(x2e0)], col = sail:::cbbPalette[c(4)], lwd = 4)
lines(x2e0[order(x2e0)],e0[order(x2e0)], col = sail:::cbbPalette[c(4)], lwd = 4, lty = 2)
lines(x2e1[order(x2e1)],e1[order(x2e1)], col = sail:::cbbPalette[c(7)], lwd = 4)
lines(x2e1[order(x2e1)],e1[order(x2e1)], col = sail:::cbbPalette[c(7)], lwd = 4, lty = 2)
legend("topleft", c("E=0", "E=1"),
